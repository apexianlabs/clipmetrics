...
const Logo = ({ size = 32 }) => (
  <svg width={size} height={size} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
    <defs><linearGradient id="lg" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor="#f472b6"/><stop offset="100%" stopColor="#be185d"/></linearGradient></defs>
    <rect x="28" y="10" width="44" height="80" rx="10" fill="url(#lg)"/>
    <rect x="32" y="18" width="36" height="56" rx="5" fill="#831843"/>
    <path d="M44 34 L44 58 L64 46 Z" fill="white" opacity="0.9"/>
  </svg>
)


const GOOGLE_AUTH_URL = `https://wdeilbhnsdlmckovicqy.supabase.co/auth/v1/authorize?provider=google&redirect_to=https%3A%2F%2Fapp.clipmetrics.app%2Fauth%2Fcallback&scopes=email+profile`
...
export default function LoginPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const body = tab === 'login'
        ? { email: form.email, password: form.password, product: 'clipmetrics' }
        : { email: form.email, password: form.password, full_name: form.name, source_product: 'clipmetrics' }
      ...
      document.cookie = `cli_token=${token}; path=/; max-age=2592000; SameSite=Lax`
      document.cookie = `cli_user=${encodeURIComponent(JSON.stringify({ id: user.id, email: user.email, name: fullName }))}; path=/; max-age=2592000; SameSite=Lax`
      ...
      if (tab === 'signup') {
        fetch(`${process.env.NEXT_PUBLIC_DB_API_URL}/email/welcome`, {
          ...
          body: JSON.stringify({ product: 'clipmetrics', to: form.email, name: fullName, email: form.email })
  ...
  const leftPanel = (
    <div style={{width:420,flexShrink:0,background:'#ec4899',display:'flex',flexDirection:'column',padding:'40px',position:'relative',overflow:'hidden'}}>
      ...
      <Link href="/" style={{display:'flex',alignItems:'center',gap:10,marginBottom:48,zIndex:1,textDecoration:'none'}}>
        <Logo size={36}/>
        <span style={{fontSize:20,fontWeight:800,color:'#fff'}}>ClipMetrics</span>
      </Link>
      <div style={{zIndex:1,marginBottom:40}}>
        ...
        <p style={{fontSize:14,color:'rgba(255,255,255,0.7)',lineHeight:1.6}}>AI social video analytics — know what works before you post</p>
      </div>
      <div style={{zIndex:1,marginTop:'auto',background:'rgba(255,255,255,0.1)',borderRadius:12,padding:20,border:'1px solid rgba(255,255,255,0.15)'}}>
        <p style={{fontSize:13,color:'rgba(255,255,255,0.9)',lineHeight:1.6,marginBottom:10,fontStyle:'italic'}}>"ClipMetrics saves me hours every week."</p>
        ...
        <div style={{width:'100%',maxWidth:400}}>
          {isMobile && (
            <Link href="/" style={{display:'flex',alignItems:'center',gap:8,marginBottom:24,textDecoration:'none',justifyContent:'center'}}>
              <Logo size={36}/>
              <span style={{fontWeight:800,fontSize:18,color:'#0f172a'}}>ClipMetrics</span>
            </Link>
          )}
          ...
          <p style={{fontSize:14,color:'#64748b',marginBottom:20}}>
            {tab === 'login' ? `Sign in to your ClipMetrics account.` : 'Start your free trial today.'}
          </p>
          ...
          <button onClick={handleSubmit} disabled={loading}
            style={{width:'100%',padding:13,borderRadius:10,border:'none',background:'#ec4899',color:'#fff',fontSize:15,fontWeight:700,cursor:'pointer',fontFamily:'Inter,sans-serif',marginBottom:16,opacity:loading?0.7:1}}>
